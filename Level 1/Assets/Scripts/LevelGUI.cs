...
public class LevelGUI : MonoBehaviour {
	...
	void OnGUI() {
		if (pause) {
			GUI.Label (new Rect( Screen.width/2-200, 30, 500, 160),     "PAUSED", Title); 
			GUI.Label(new Rect(300, 200, 1000, 500), keyboard);
			GUI.Label (new Rect( Screen.width-600, Screen.height - 360, 400, 70),     "Movement:  WASD or Arrow Keys", pauseStyle); 
			GUI.Label (new Rect( Screen.width-600, Screen.height - 400, 400, 70),     "Ability:  Shift", pauseStyle); 
			GUI.Label (new Rect( Screen.width-600, Screen.height - 450, 400, 70),     "Jump:  Spacebar", pauseStyle); 
			GUI.Label (new Rect( Screen.width-600, Screen.height - 500, 400, 70),     "Age:  Q", pauseStyle); 
			GUI.Label (new Rect( Screen.width-600, Screen.height - 550, 400, 70),     "Rebirth:  R", pauseStyle); 
			GUI.Label (new Rect( Screen.width-600, Screen.height - 600, 400, 70),     "Pause/Unpause:  P", pauseStyle); 
		}
		else {
			if (inv.hasItem ("Key")) {
				inventory[0] = keyTex;	
